...
import { Link2, Menu, Minus, Plus, X } from "lucide-react";
...
export function CanvasLabPage({ engagementId }: { engagementId: string }) {
  ...
  function cancelConnect(note = true) {
    if (note && connectMode) noteWorkboardRelationship(orgId, "cancelled");
  ...
  useEffect(() => {
    function onKey(event: KeyboardEvent) {
      if (event.key === "Escape") {
        ...
        else if (connectMode) cancelConnect();
  ...
  }, [allNodes, connectMode, focusId, keyboardId, menuOpen, orgId, reviewId, selectedLinkId]);
  ...
  useEffect(() => {
    ...
    function up() { dragRef.current = null; panRef.current = null; }
  ...
  function onCardKeyDown(node: LabNode, event: React.KeyboardEvent) {
    if (event.key === "Enter" || event.key === " ") {
      event.preventDefault();
      if (connectMode) chooseConnectNode(node);
      else setSelected((current) => toggleContext(current, node.id));
      return;
    }
  ...
  function chooseConnectNode(node: LabNode) {
    if (!connectSourceId) {
      setConnectSourceId(node.id);
      setAnnouncement(`${node.title} chosen as the source. Choose a target.`);
      return;
    }
    const result = addLabLink(links, connectSourceId, node.id);
    if (result.error) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex bg-[var(--nb-paper)]" data-testid="canvas-lab-shell">
        ...
        <div ref={shellRef} onPointerDown={(event) => { if (event.button === 0 && event.target === event.currentTarget) panRef.current = { from: { x: event.clientX, y: event.clientY }, origin: pan }; }} className="canvas-lab-surface relative min-h-0 flex-1 cursor-grab overflow-hidden">
          <div data-testid="canvas-lab-stage" className="absolute left-0 top-0 origin-top-left" style={{ width: bounds.width, height: bounds.height, transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})` }}>
            ...
            <svg className="absolute inset-0 overflow-visible" width={bounds.width} height={bounds.height} aria-label="Local workboard relationships">
              ...
              {links.map((link) => { const source = visibleNodes.find((node) => node.id === link.fromId); const target = visibleNodes.find((node) => node.id === link.toId); if (!source || !target) return null; const sx = source.x + 232; const sy = source.y + 54; const tx = target.x; const ty = target.y + 54; const middle = (sx + tx) / 2; const active = selectedLinkId === link.id; return <path key={link.id} role="button" tabIndex={0} aria-label={`Select relationship from ${source.title} to ${target.title}`} onClick={() => setSelectedLinkId(link.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelectedLinkId(link.id); } }} d={`M ${sx} ${sy} C ${middle} ${sy}, ${middle} ${ty}, ${tx} ${ty}`} fill="none" stroke={active ? "var(--nb-green)" : "var(--nb-graphite)"} strokeWidth={active ? "2.4" : "1.4"} strokeLinecap="round" className="cursor-pointer outline-none focus:stroke-[var(--nb-green)]" />; })}
            </svg>
            {visibleNodes.map((node) => <LabCard key={node.id} node={node} item={itemByNode(node)} selected={selected.includes(node.id)} focused={keyboardId === node.id} connecting={connectMode} connectSource={connectSourceId === node.id} onSelect={() => setSelected((current) => toggleContext(current, node.id))} onOpen={() => openNode(node)} onBranch={() => branchFrom(node)} onHide={() => hideNode(node)} onDelete={() => deleteNode(node)} onEdit={(text) => setNodes((current) => current ? updateLocalNode(current, node.id, text) : current)} onEditCommitted={() => noteWorkboardNodeEdited(orgId, eventKind(node))} onConnect={() => chooseConnectNode(node)} onPointerDown={(event) => onCardPointerDown(node, event)} onKeyDown={(event) => onCardKeyDown(node, event)} />)}
          </div>
      ...
      {reviewItem ? <CanvasLabReview item={reviewItem} profileId={profile?.id} decisions={page?.decisions ?? []} nodes={visibleNodes} comments={comments} onTrailSelect={(group, focus) => noteWorkboardTrailSelected(orgId, group, focus)} onClose={() => setReviewId(null)} /> : null}
    </div>
  );
}
