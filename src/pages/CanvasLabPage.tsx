...
import { WORKBOARD_SHAPE_COLOURS, serializeWorkboardTextBody, type WorkboardCommand, type WorkboardNodeInput, type WorkboardRelation, type WorkboardShapeColour, type WorkboardTextBody } from "@/lib/canvas-lab-shared";
...
export function CanvasLabPage({ engagementId, entryVia }: { engagementId: string; entryVia?: string }) {
  ...
  async function materialize(): Promise<boolean> {
    ...
    const frameInputs = framesRef.current.map((frame, index) => ({
      ...
      label: frameKindOf(frame) === "custom" ? frame.name : frameKindOf(frame) === "context" ? CONTEXT_FRAME_LABEL : null,
      x: frame.x,
      y: frame.y,
      w: frame.width,
      h: frame.height,
  ...
  useEffect(() => {
    function onKey(event: KeyboardEvent) {
      ...
      if (!node) return;
      if (!node.local && node.kind !== "chat" && node.kind !== "shape" && node.kind !== "text") {
        event.preventDefault();
