...
export function useDecisionActions(surface: DecisionSurface = "log") {
  ...
  async function apply(decision: DecisionRow, patch: Patch, status: string, edited: boolean) {
    ...
    if (profile) {
      ...
      if (status === "confirmed")
        logEvent("decision.confirmed", profile.org_id, { edited, surface });
      logV2("decision.resolved", { status: status as never, edited }, { profileId: profile.id });
      if (status === "confirmed")
        logV2(
          "decision.confirmed",
          { edited, evidence_count: 0, surface },
          { profileId: profile.id },
        );
      if (status === "discarded")
        logV2("decision.discarded", { edited, surface }, { profileId: profile.id });
      if (edited) logV2("decision.edited", { edited, surface }, { profileId: profile.id });
    }
