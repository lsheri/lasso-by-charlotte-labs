...
export function SetupTools({ tools }: { tools: ToolId[] }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {mcp.length > 0 ? (
        <section className="space-y-3">
          <h2 className="micro-label text-graphite">Let your AI push to Lasso</h2>
          {mcp.map((id) => (
      ...
      {guides.length > 0 ? (
        <section className="space-y-3">
          <h2 className="micro-label text-graphite">Export and import</h2>
          {guides.map((id) => (
      ...
      <section className="space-y-3">
        <h2 className="micro-label text-graphite">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <PasteThreadDialog
            trigger={
              <button
                type="button"
                className="rounded-[var(--radius)] border border-border bg-card p-4 text-left shadow-card transition-colors hover:border-graphite"
              >
