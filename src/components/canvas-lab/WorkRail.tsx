...
export function WorkRail({ open, mobileVisible, context, hidden, canvasInstructions, onToggle, onRemoveContext, onSubmit, onCanvasInstructions, onRestore, onShowBoard }: { open: boolean; mobileVisible: boolean; context: LabNode[]; hidden: LabNode[]; canvasInstructions: string; onToggle: () => void; onRemoveContext: (id: string) => void; onSubmit: (prompt: string) => void; onCanvasInstructions: (value: string) => void; onRestore: (id: string) => void; onShowBoard: () => void }) {
  if (!open) return <aside className="canvas-lab-rail-collapsed"><Button type="button" variant="ghost" size="sm" className="h-auto min-h-32 w-10 px-1 [writing-mode:vertical-rl]" aria-label="Open working from rail" onClick={onToggle}><Paperclip className="mb-2 h-4 w-4" />Working from</Button></aside>;
  return (
    <aside className={`canvas-lab-work-rail ${mobileVisible ? "flex" : "hidden md:flex"}`} aria-label="Working from">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3"><ContextComposer context={context} onRemoveContext={onRemoveContext} onSubmit={onSubmit} canvasInstructions={canvasInstructions} onCanvasInstructions={onCanvasInstructions} /></div>
