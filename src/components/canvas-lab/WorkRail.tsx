...
export function WorkRail({ open, mobileVisible, context, hidden, canvasInstructions, onToggle, onRemoveContext, onClearContext, onSubmit, onCanvasInstructions, onRestore, onShowBoard }: { open: boolean; mobileVisible: boolean; context: LabNode[]; hidden: LabNode[]; canvasInstructions: string; onToggle: () => void; onRemoveContext: (id: string) => void; onClearContext: () => void; onSubmit: (prompt: string) => void; onCanvasInstructions: (value: string) => void; onRestore: (id: string) => void; onShowBoard: () => void }) {
  return (
    <aside className={`${open ? "canvas-lab-work-rail" : "canvas-lab-rail-collapsed"} ${mobileVisible ? "flex" : "hidden md:flex"}`} aria-label="Working from">
      ...
      <div hidden={!open} className="min-h-0 flex-1 overflow-y-auto p-3"><ContextComposer context={context} onRemoveContext={onRemoveContext} onClearContext={onClearContext} onSubmit={onSubmit} canvasInstructions={canvasInstructions} onCanvasInstructions={onCanvasInstructions} /></div>
      <details hidden={!open} className="border-t border-border p-3"><summary className="cursor-pointer font-mono text-[9px] uppercase tracking-[0.08em] text-green">Add from engagement · {hidden.length}</summary>{hidden.length === 0 ? <p className="mt-2 nb-type-small text-muted-foreground">Nothing has been removed from this board.</p> : <ul className="mt-2 space-y-1">{hidden.map((node) => <li key={node.id}><Button type="button" size="sm" variant="ghost" className="h-auto w-full justify-start text-left nb-type-small" onClick={() => onRestore(node.id)}><Plus className="mr-1 h-3 w-3" />{node.title}</Button></li>)}</ul>}</details>
    </aside>
  );
}