...
export function LabTextBlock({ node, selected, editable, onSelect, onDragStart, onResizeStart, onResizeKeyDown, onResizeKeyUp, onChange, onCommit, onRemove }: {
  ...
  return (
    <section data-testid={`text-block-${node.id}`} data-selected={selected} data-empty={body.text.length === 0} data-size={body.size} data-weight={body.weight} data-colour={body.colour} aria-label="Text block" tabIndex={selected ? 0 : -1} className="canvas-lab-text-block absolute" style={style} onFocus={onSelect} onPointerDown={(event) => { if (selected) onDragStart(event); }}>
      ...
      <textarea aria-label="Text block words" maxLength={WORKBOARD_TEXT_MAX_LENGTH} value={body.text} readOnly={!editable} placeholder="Type something" onPointerDown={(event) => event.stopPropagation()} onChange={(event) => change({ text: event.target.value })} onBlur={onCommit} />
      {selected && editable ? <div className="canvas-lab-text-block-controls" onPointerDown={(event) => event.stopPropagation()}>
        <select aria-label="Text size" value={body.size} onChange={(event) => { change({ size: event.target.value as WorkboardTextBody["size"] }); queueMicrotask(onCommit); }}>{WORKBOARD_TEXT_SIZES.map((value) => <option key={value} value={value}>{value}</option>)}</select>
        <select aria-label="Text weight" value={body.weight} onChange={(event) => { change({ weight: event.target.value as WorkboardTextBody["weight"] }); queueMicrotask(onCommit); }}>{WORKBOARD_TEXT_WEIGHTS.map((value) => <option key={value} value={value}>{value}</option>)}</select>
        <select aria-label="Text colour" value={body.colour} onChange={(event) => { change({ colour: event.target.value as WorkboardTextBody["colour"] }); queueMicrotask(onCommit); }}>{WORKBOARD_TEXT_COLOURS.map((value) => <option key={value} value={value}>{value}</option>)}</select>
