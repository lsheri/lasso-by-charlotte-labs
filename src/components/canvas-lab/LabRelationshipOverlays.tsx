...
export function LabRelationshipOverlays({ links, nodes, measuredHeights, selectedLinkId, hoveredLinkId, inverseZoom, zoom, editable, onRemove, onChangeRelation, onHover }: {
  links: LabLink[];
  nodes: LabNode[];
  ...
  return <g aria-label="Workboard relationship labels and controls">
    {links.map((link) => {
      ...
      return <g key={link.id}>
        ...
        {editable && emphasized ? <foreignObject x={point.x - controlsWidth / 2} y={point.y - removeSize / 2} width={controlsWidth} height={removeSize} className="canvas-lab-relationship-remove-wrap" onPointerEnter={() => onHover?.(link.id)} onPointerLeave={() => onHover?.(null)}>
