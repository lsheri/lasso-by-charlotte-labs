          ...
          <div key={step.kind} className="relative min-w-[124px] border border-dashed border-[var(--nb-pencil)] bg-card p-2">
            ...
            {step.kind === "judgment" ? (
              <details className="mt-2">
                <summary className="cursor-pointer font-mono text-[9px] uppercase tracking-[0.08em] text-green">Add</summary>
                <div className="absolute left-0 top-full z-20 mt-1 w-48 border border-border bg-card p-1 shadow-[var(--shadow-modal)]">
                  {JUDGMENT_TYPES.map((choice) => (
                    <Button key={choice.value} type="button" variant="ghost" size="sm" className="w-full justify-start text-[11px]" onClick={() => onAdd("judgment", choice.value)}>{choice.label}</Button>
                  ))}
                </div>
              </details>
            ) : (
