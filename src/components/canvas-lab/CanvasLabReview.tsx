...
export function CanvasLabReview({ item, anchorNodeId, links, profileId, decisions, nodes, comments, onTrailSelect, onClose }: { item: WorkItemRow; anchorNodeId: string; links: LabLink[]; profileId?: string | undefined; decisions: DecisionRow[]; nodes: LabNode[]; comments: LabComment[]; onTrailSelect: (group: TrailGroup, focus: "exact" | "item") => void; onClose: () => void }) {
  ...
  const connectedNodeIds = useMemo(() => inboundLabNodeIds(nodes, links, anchorNodeId), [anchorNodeId, links, nodes]);
  const relatedLocalJudgments = nodes.filter((node) => node.kind === "judgment" && connectedNodeIds.has(node.id));
  /** The word the person chose for a direct link, rather than a generic one. */
  const relationWord = (nodeId: string): string => {
    const direct = links.find((link) => (link.fromId === nodeId && link.toId === anchorNodeId) || (link.toId === nodeId && link.fromId === anchorNodeId));
    const relation = direct?.relation ?? "context";
    return relation === "context" ? "" : ` · ${relation}`;
  };
  ...
  return <div className="fixed inset-0 z-[60] flex flex-col bg-background" data-testid="canvas-lab-review">
    ...
    <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
      <aside className="w-full shrink-0 overflow-y-auto border-b border-border bg-[var(--nb-paper)] p-3 lg:w-[280px] lg:border-b-0 lg:border-r">
        ...
        {isError && !isLoading ? <p className="text-[12px] text-muted-foreground">This reasoning trail could not be opened.</p> : null}
        {ready ? <>
        {groups.map((group) => <section key={group.id} className="mb-4"><h2 className="font-hand text-[16px] text-green">{group.label}</h2>{group.rows.length === 0 ? <p className="mt-1 nb-type-small text-muted-foreground">Nothing attached here.</p> : <ul className="mt-1 space-y-1">{group.rows.map((entry) => <li key={entry.id}><Button type="button" variant="ghost" size="sm" className="h-auto w-full justify-start whitespace-normal text-left nb-type-small" onClick={() => { const firstTurn = entry.turns[0]; setSourceId(entry.id); setFocus(firstTurn ? { turnNo: firstTurn.turn_no, text: firstTurn.content } : undefined); setActiveStitchId(null); onTrailSelect(group.id, firstTurn ? "exact" : "item"); }}><SourceMark item={{ source_vendor: entry.source_vendor }} size={12} />{entry.title}</Button></li>)}</ul>}</section>)}
        <section className="mb-4"><h2 className="font-hand text-[16px] text-green">Human judgment</h2>{judgmentEmpty ? <p className="mt-1 nb-type-small text-muted-foreground">Nothing attached here.</p> : null}<ul className="mt-1 space-y-1">{(data?.stitches ?? []).map((stitch) => <li key={stitch.id}><Button type="button" variant="ghost" size="sm" className="h-auto w-full justify-start whitespace-normal text-left nb-type-small" onClick={() => { setActiveStitchId(stitch.id); setSourceId(null); setFocus(undefined); onTrailSelect("human_judgment", stitch.locator?.snippet ? "exact" : "item"); }}>{stitch.question || stitch.quote || "Recorded question"}</Button></li>)}{relatedLocalJudgments.map((node) => <li key={node.id} className="border-l-2 border-[var(--nb-green)] pl-2 nb-type-small">{node.title}{relationWord(node.id)}{node.durableId ? "" : " · Not saved"}</li>)}{relatedLocalComments.map((comment) => <li key={comment.id} className="border-l-2 border-[var(--nb-green)] pl-2 nb-type-small">{comment.body} · Not saved</li>)}</ul></section>
        <section><h2 className="font-hand text-[16px] text-green">Decisions</h2>{relatedDecisions.length === 0 ? <p className="mt-1 nb-type-small text-muted-foreground">No decision with an attached source appears here.</p> : <ul className="mt-1 space-y-1">{relatedDecisions.map((decision) => <li key={decision.id}><Button type="button" variant="ghost" size="sm" className="h-auto w-full justify-start whitespace-normal text-left nb-type-small" onClick={() => { const source = srcsOf(decision).find((entry) => sourceIds.has(entry.work_item_id)); if (source) { const sourceItem = (data?.upstream ?? []).find((entry) => entry.id === source.work_item_id); const turn = sourceItem?.turns.find((entry) => entry.id === source.turn_id); setSourceId(source.work_item_id); setFocus(turn ? { turnNo: turn.turn_no, text: turn.content } : undefined); } onTrailSelect("decisions", source?.turn_id ? "exact" : "item"); }}>{decision.call_text}</Button></li>)}</ul>}</section>
