...
export function LabPreview({ item, preview, filePreview, focused = false, onFailure, onPreviewScroll, onOpen }: { item: WorkItemRow; preview?: WorkboardCardPreview | undefined; filePreview?: FilePreview | undefined; focused?: boolean; onFailure: () => void; onPreviewScroll?: ((kind: "chat" | "document" | "deck" | "html") => void) | undefined; onOpen: () => void }) {
  ...
  return <div data-drawing="preview" data-preview-shape={shape} className="nb-paper canvas-lab-preview-frame">
    ...
    <div className="nb-preview-content min-h-0 flex-1">
      {item.type === "ai_thread" ? <ChatPreviewWindow testId="workboard-chat-preview" vendorKey={sourceVendorKey(item)} turns={preview?.turns ?? []} /> : filePreview ? <div className="nb-document-preview-body h-full"><WorkboardFilePreview preview={filePreview} title={item.title} focused={focused} onFailure={onFailure} onPageChange={() => onPreviewScroll?.(filePreview.kind === "html" ? "html" : item.type === "deck" ? "deck" : "document")} /></div> : null}
    </div>
  </div>;
}