...
describe("what a link means", () => {
  it("writes the word at the midpoint when the board is readable", () => {
    const { container } = render(<svg><LabRelationships links={[link]} nodes={nodes} measuredHeights={new Map()} selectedLinkId={null} inverseZoom={1} zoom={1} editable onSelect={() => undefined} onRemove={() => undefined} /></svg>);
  ...
  it("stays quiet below 0.6 and for context", () => {
    const small = render(<svg><LabRelationships links={[link]} nodes={nodes} measuredHeights={new Map()} selectedLinkId={null} inverseZoom={2} zoom={0.5} editable onSelect={() => undefined} onRemove={() => undefined} /></svg>);
    expect(small.container.querySelector("[data-testid='lab-relationship-label-link']")).toBeNull();
    cleanup();
    const plain = render(<svg><LabRelationships links={[{ ...link, relation: "context" }]} nodes={nodes} measuredHeights={new Map()} selectedLinkId={null} inverseZoom={1} zoom={1} editable onSelect={() => undefined} onRemove={() => undefined} /></svg>);
  ...
  it("offers a change control beside the remove control when selected", () => {
    const change = vi.fn();
    render(<svg><LabRelationships links={[link]} nodes={nodes} measuredHeights={new Map()} selectedLinkId="link" inverseZoom={1} zoom={1} editable onSelect={() => undefined} onRemove={() => undefined} onChangeRelation={change} /></svg>);
