...
describe("Canvas Lab card interaction correction", () => {
  ...
  it("keeps resize, structure, and reassignment bounded to the workboard", () => {
    ...
    expect(page).toContain('useState<LabStructureMode>("freeform")');
  ...
  it("shares workstream creation between the drawer and inline controls", () => {
    const page = read("src/pages/CanvasLabPage.tsx");
    expect(page).toContain("function addWorkstream(nameInput: string): boolean");
    expect(page).toContain("addWorkstream(newFrameName)");
    expect(page).toContain("addWorkstream(inlineFrameName)");
    expect(page).toContain('onAddWorkstream={frame.id === "workstreams" ? addWorkstream : undefined}');
  });
