    ...
    <div className="demo-playground" data-resetting={resetting}>
      ...
      <div className="demo-play-canvas" data-ask-open={askOpen}>
        <div className="demo-play-board" onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag}>
          ...
          {stickies.map((sticky) => <article key={sticky.id} data-testid="demo-play-sticky" className="demo-play-sticky" style={{ left: sticky.x, top: sticky.y, transform: transform(sticky.id) }} onPointerDown={(eventValue) => beginDrag(eventValue, sticky.id, "drag_card")}><input value={sticky.text} maxLength={80} aria-label="Sticky note" onFocus={() => { clearResetTimer(); setEditing(true); }} onBlur={() => { setEditing(false); emit("sticky_edited"); scheduleReset(); }} onChange={(eventValue) => setStickies((current) => current.map((entry) => entry.id === sticky.id ? { ...entry, text: eventValue.target.value.slice(0, 80) } : entry))} /></article>)}
        </div>
  ...
  return (
    <aside className="lb-turn-reader" aria-label="Exact turn reader">
      ...
      <div ref={bodyRef} className="lb-turn-body" data-testid="landing-decision-transcript">
        {turns.filter((turn) => !phone || turn.turn_no >= 4).map((turn) => {
...
function PhoneStory({ board, presets, proof, active, onActive, onWatch, onPilot, onShowSlide, onOpenTurn, onOpenDecisionTurn }: { board: SharedBoardDto; presets: DemoPreset[]; proof: LandingProof | null; active: number; onActive: (index: number) => void; onWatch: () => void; onPilot: () => void; onShowSlide: () => void; onOpenTurn: () => void; onOpenDecisionTurn: () => void }) {
  ...
  return <div ref={rootRef} className="lb-phone-story" data-active-step={active + 1}>
    {LANDING_BOARD_STEPS.map((step, index) => <section key={step.key} id={`lb-phone-${step.key}`} className="lb-phone-step" data-phone-step={index} data-entered={index === 0 ? "true" : undefined}>
      ...
      <div className="lb-phone-visual">
        ...
        {index === 3 ? <PhoneDeck clientLabel={board.seed.clientLabel ?? ""} proof={proofModel} /> : null}
        {index === 4 ? <PhoneDeck clientLabel={board.seed.clientLabel ?? ""} proof={proofModel} lasso /> : null}
