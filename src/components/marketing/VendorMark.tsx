/** Brand marks for the four AI products shown in the collage: real logos. */
import claudeLogo from "@/assets/claude-logo.png.asset.json";
import lovableLogo from "@/assets/lovable-logo.png.asset.json";

export type VendorKey = "chatgpt" | "claude" | "gemini" | "lovable";

export function VendorMark({ vendor, size = 24 }: { vendor: VendorKey; size?: number }) {
  if (vendor === "claude") {
    return (
      <img
        src={claudeLogo.url}
  ...
  if (vendor === "chatgpt") {
    ...
    return (
      <svg {...common} fill="var(--nb-graphite)">
  ...
  // The official Gemini spark, in its blue-to-violet gradient.
  return (
    <svg {...common}>
      <defs>
        <linearGradient id="nb-gemini-grad" x1="0" y1="24" x2="24" y2="0">
