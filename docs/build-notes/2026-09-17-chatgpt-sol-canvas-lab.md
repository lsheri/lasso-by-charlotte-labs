...
## 2026-09-19 — Workboard frame menu, rename, and remove

- **Controls:** every existing frame control remains: frame selection, the Fit button, four corner resize handles with pointer and keyboard resizing, count, and local label. Custom workstreams add one ellipsis/context menu with Fit contents, Rename, and Remove workstream, plus a 60-character inline rename field.
